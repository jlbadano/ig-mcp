...
class InstagramMCPSettings(BaseSettings):
    ...
    instagram_api_base_url: str = Field("https://graph.facebook.com", description="Instagram API base URL")
    
    # Rate Limiting Configuration
    rate_limit_requests_per_hour: int = Field(200, description="Rate limit requests per hour")
    rate_limit_posts_per_day: int = Field(25, description="Rate limit posts per day")
    rate_limit_enable_backoff: bool = Field(True, description="Enable rate limit backoff")
    
    # Logging Configuration
    log_level: str = Field("INFO", description="Log level")
    log_format: str = Field("json", description="Log format")
    log_file: Optional[str] = Field("logs/instagram_mcp.log", description="Log file path")
    ...
    database_url: Optional[str] = Field("sqlite:///instagram_mcp.db", description="Database URL")
    database_echo: bool = Field(False, description="Database echo")
    
    # Optional: Webhook Configuration
    webhook_verify_token: Optional[str] = Field(None, description="Webhook verify token")
